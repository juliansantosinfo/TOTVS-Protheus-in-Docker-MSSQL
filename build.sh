...
# ==============================================================================
# SCRIPT: build.sh
# DESCRIÇÃO: Responsável por realizar o build da imagem Docker para o MSSQL 
#            com TOTVS Protheus.
...
# ----------------------------------------------------
    ...
    MSSQL_BACKUP_DIR="./packages"
    MSSQL_BACKUP_FILES=(
        "${MSSQL_BACKUP_DIR}/data.tar.gz"
    )
    ...
    if [[ -n "$CUSTOM_TAG" ]]; then
        ...
        DOCKER_TAG="${DOCKER_USER}/${MSSQL_IMAGE_NAME}:${MSSQL_VERSION}"
    fi
