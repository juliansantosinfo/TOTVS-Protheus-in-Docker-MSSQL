...
# ==============================================================================
# SCRIPT: push.sh
# DESCRIÇÃO: Envia a imagem Docker do MSSQL TOTVS Protheus para o Docker Hub.
...
# ----------------------------------------------------
    ...
    if [[ -n "$CUSTOM_TAG" ]]; then
        ...
        FULL_TAG="${DOCKER_USER}/${MSSQL_IMAGE_NAME}:${MSSQL_VERSION}"
    fi

    LATEST_TAG="${DOCKER_USER}/${MSSQL_IMAGE_NAME}:latest"

    # Validação de variáveis
    if [[ -z "${MSSQL_VERSION:-}" ]] || [[ -z "${MSSQL_IMAGE_NAME:-}" ]] || [[ -z "${DOCKER_USER:-}" ]]; then
